<template>
    <v-tooltip right :disabled="!data || !data.items || !data.items[0] || !data.items[0].settings || !data.items[0].settings.tooltip">
         <template v-slot:activator="{ on, attrs }">
            <div :class="['trading-vue-tbitem', selected ? 'selected-item' : '']"
                ...
                <v-icon class="trade-plan-premium-warning"
                    v-if="(config.USER_TYPE === 'new' || config.USER_TYPE === 'freemium') && (config.USER_TRADE_PLAN_COUNT === 10) && data.group === 'Trade Plan'">$redIcon</v-icon>
        ...
        <span v-if="data && data.items">{{data.items[0].settings.tooltip}}</span>
    </v-tooltip>
</template>
...
<style>
.trade-plan-premium-warning.v-icon{
    width: 15px;
    height: 15px;
    position: absolute;
    right: 10px;
