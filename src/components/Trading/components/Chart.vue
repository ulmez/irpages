...
export default {
    ...
    props: [
        ...
        'skin', 'timezone', 'page', 'showrightSideBarLine'
    ],
    ...
    methods: {
        ...
        default_range() {
            const defaultDataLen = this.$props.config.DEFAULT_LEN
            const minimumDaysLen = this.$props.config.MINIMUM_LEN + 0.5
            const last = this.ohlcv.length - 1
            let start = 0, days = 0

            if (this.ohlcv.length < 2) return
            if (this.ohlcv.length <= defaultDataLen) {
                start = 0, days = minimumDaysLen
            } else {
                start = last - defaultDataLen, days = 0.5
            }
            if (!this.$props.ib) {
                Utils.overwrite(this.range, [
                    this.ohlcv[start][0] - this.interval * days,
                    this.ohlcv[last][0] + this.interval * minimumDaysLen
                ])
            } else {
                Utils.overwrite(this.range, [
                    start - this.interval * days,
                    last + this.interval * minimumDaysLen
                ])
            }
            /**
             * if the page is 'watchlist' and it has a 'Trade plan'
             * then get min days from there and apply it as range.
             */
            // if(this.page === 'wl' && this.onchart.length > 0 &&  this.onchart.filter((elm) => elm.type === 'PositionTool').length > 0) {
            if(this.page === 'wl') {
                const positionToolList = this.onchart.filter((elm) => elm.type === 'PositionTool');
                const dateList = positionToolList.map((elm) => {
                    return [elm.settings.p1[0], elm.settings.p2[0]]
                }).flat();
                Utils.overwrite(this.range, [
                    Math.min.apply(null, dateList) - this.interval * minimumDaysLen,
                    Utils.now() + this.interval * 90
                ])
            }
            /**
             * if the page is 'overview
             * then set min and max days of range from -4 months ~ +2 months.
             */
            // if((this.page === 'overview' || this.page === 'dropdown' || this.page === 'none') && this.onchart.length > 0 &&  this.onchart.filter((elm) => elm.type === 'PositionTool').length > 0) {
            if((this.page === 'overview' || this.page === 'dropdown' || this.page === 'none')) {
                Utils.overwrite(this.range, [
                    Utils.now() - this.interval * 120,
                    Utils.now() + this.interval * 90
                ])
            }
        },
